...
private:
  void Init() {
    ...
    m_shader_id = (m_shader_type == ShaderType::VERTEX_SHADER)
    ...
    if (!success) {
      ...
      std::cout << ((m_shader_type == ShaderType::VERTEX_SHADER)
                        ? "VertexShader"
                        : "FragmentShader")
                << errorInfo << std::endl;
    }
...
public:
  Shader(std::string file, ShaderType type)
      : m_file(file), m_shader_type(type) {
    Init();
  }
...
public:
  ...
  void SetUniform1f(std::string name, float shininess) {
    auto location = glGetUniformLocation(m_program_id, name.c_str());
    this->Use();
