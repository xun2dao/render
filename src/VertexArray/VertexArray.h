...
public:
  Buffer() {}
  Buffer(GLenum type, T *data, unsigned int count)
      : m_type(type), m_data(data), m_count(count) {
  ...
  void TransferData(T *data, unsigned int count) {
    ...
    glBufferData(m_type, sizeof(T) * count, data, GL_STATIC_DRAW);
  }
  void Use() { glBindBuffer(m_type, m_buffer_id); }
...
public:
  VertexArray() { Init(0, nullptr, 0, nullptr); }
  VertexArray(unsigned int count, float *data, unsigned int point_nums = 0,
              unsigned int *index = nullptr) {
    Init(count, data, point_nums, index);
  }
  ...
  void Init(unsigned int count, float *data, unsigned int point_nums = 0,
            unsigned int *index = nullptr) {
  ...
  void Use() { glBindVertexArray(m_vao_id); }

  void Unuse() { glBindVertexArray(0); }
